...
# Render function to visualize set render to True
render = False
...
def epsilon_greedy(Q, state, nA, epsilon = 0.1):
    ...
    weights = np.ones(nA,dtype=float)*epsilon/nA
    greedy_action = np.argmax(Q[state])
    weights[greedy_action] +=(1-epsilon)
    
    action = random.choices(action,weights,k=1)[0]
